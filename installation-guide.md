...
[Red Hat® AMQ streams](https://www.redhat.com/en/resources/amq-streams-datasheet) is a massively scalable, distributed, and high-performance data streaming platform based on the Apache Kafka project.
...
1. JDK - JDK needs to be install on all servers. Use this command to install Open JDK.
...
The script can be used to generate both [wildcard certificate](https://en.wikipedia.org/wiki/Wildcard_certificate) and regular/standalone certificate with [Subject Alternative Name](https://en.wikipedia.org/wiki/Subject_Alternative_Name) or `SAN` extension. For wildcard certificate, the `SAN` extention will have `DNS` attribute with a value as the wildcard domain name i.e. `*.example.com`. For regular/standalone certificate, the `SAN` extenstion will have both `DNS` attribute with value a as the FQDN of each server i.e. `kafka1.example.com` and `IP` attribute with a value as the IP address of each server in the cluster.
...
2. Run the script to create all necessary files i.e. truststore, keystore.
   ...
   - For **regular/standalone** certificate generation, because a key pairs and public key certificate need to be created individually for each server so the script requires a list of server's host name and IP address. Hence, you have to set a `HOST_IP_LIST` variable with value of a list of hostname (without domain) and IP address pairs in this format: `hostname:ipaddress;hostname:ipaddress;...hostname:ipaddress` before running the script.
     ...
     **_Note:_** Only `keystore.jks` and `truststore.jks` are needed for SSL/TLS setup for the cluster. Though, you might need to keep `ca.crt` (CA's certificate) and `ca.key` (CA's private key) in a safe place for signing any new certificate in the future or you can delete them if not required.
...
5. Update the `zookeeper.connect` property in the `server.properties` file in `/opt/redhat-amq-streams/config` directory on server by replacing the FQDN of Zookeeper servers within your cluster.

   ```text
   zookeeper.connect=zookeeper1.example.com:2182,zookeeper2.example.com:2182,zookeeper3.example.com:2182
   ```

6. Change `/opt/redhat-amq-streams` directory's ownership to the `kafka` user and group.

   ```sh
   chown -R kafka:kafka /opt/redhat-amq-streams
   ```

7. Upload [`kafka.service`](kafka.service) file to `/etc/systemd/system` directory. This will make Kafka to be able to start as a system daemon using `systemctl` command.

8. Reload and enable Kafka daemon.
...
9. Start Kafka as a daemon.

   ```sh
   systtemctl start kafka
   ```

10. Make sure that Kafka is running.
...
In practice, howerver, users with limited permissions might need to be created properly. To do so, you can add more users to the `KafkaServer` section based on your requirements. Any new user must be added with this format `user_<username>="<password>"` i.e. `user_kafkaclient="client-secret"`.
